...
export async function POST(request: NextRequest) {
  try {
    ...
    // Insert job using service role key (bypasses RLS)
    ...
    // Add activity tracking for job creation
    await addActivity(`New job "${data.title}" created`, 'job_created', `Job at ${data.company_name}`)
...
export async function GET() {
  try {
    // Fetch all jobs using service role key
    const { data, error } = await supabaseAdmin
      .from('jobs')
      .select('*')
